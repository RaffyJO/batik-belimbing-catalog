    ...
    @foreach ($activities as $activity)
        <figure class="flex-shrink max-w-full px-3 w-full sm:w-1/2 lg:w-1/4 group wow fadeInUp" data-wow-duration="1s">
            <div class="relative overflow-hidden cursor-pointer mb-6">
                <a href="{{ Storage::url($activity->image) }}" data-gallery="gallery1"
                    data-glightbox="title: Judul ; description: Kegiatan seru "
                    class="glightbox3">
                    <img class="block w-full h-auto transform duration-500 grayscale hover:scale-125"
                        src="{{ Storage::url($activity->image) }}" alt="Image Description">
                    <div
                        ...
                        <h3 class="text-base leading-normal font-semibold my-1 text-white">Judul</h3>
                        <small class="d-block">Kegiatan seru</small>
                    </div>
                </a>
            </div>
        </figure>
